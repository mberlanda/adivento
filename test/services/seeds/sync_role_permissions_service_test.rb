...
module Seeds
  class SyncRolePermissionsServiceTest < ActiveSupport::TestCase
    setup do
    ...
    test 'creates desired role permissions and prunes extra mappings' do
      ...
      assert_not RolePermission.exists?(role_name: 'player', permission_id: permission_manage.id)

      market_create = Permission.find_by!(key: 'market.create')

      assert RolePermission.exists?(role_name: 'admin', permission_id: market_create.id)
    end
    ...
    test 'raises when catalog exposes invalid roles' do
      ...
      singleton.class_eval do
        ...
        define_method(:catalog_roles) { %w[admin ghost] }
