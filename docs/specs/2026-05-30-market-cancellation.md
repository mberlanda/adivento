...
4. **clob** — two parts:
   ...
   b. **Filled positions**: refund each user their **net cash outlay** for the market. Both fill types put cash in: a cross-side buy match (`execute_fill!`) writes `ORDER_FILL_STAKE` for the taker **and** consumes the resting maker's reservation, recorded as `ORDER_FILL_CREDIT` (the maker's stake); a sell match (`execute_sell_fill!`) writes `ORDER_FILL_STAKE` for the buyer and `CLOB_SELL_CREDIT` (cash out) for the seller. So net outlay = `Σ(ORDER_FILL_STAKE + ORDER_FILL_CREDIT) − Σ(CLOB_SELL_CREDIT)`. Net buyers receive a refund (`MARKET_CANCEL_REFUND` credit); net sellers (who already extracted cash) are clawed back (`MARKET_CANCEL_CLAWBACK` debit), floored at their available balance, with any shortfall recorded in the cancellation AuditEvent metadata as `clawback_shortfall_minor`. `CLOB_FEE` is not refunded. **Refunds and clawbacks are reported separately** (`refunded_total_minor` is never netted against clawbacks).
