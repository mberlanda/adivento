/* Adivento DS — React charts (pure SVG, theme-aware via CSS vars)
   These read --ds-* at render; wrap in a key that changes on theme to recolor,
...
export function Chart({ points = [], color = "var(--ds-accent)", height = 170, showAxis = true }) {
  const W = 300, H = 120;
  if (points.length < 2) return <div className="ds-chart" style={{ height }} />;
  ...
  return (
    <div className="ds-chart" style={{ height }}>
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none">
        {[25, 50, 75].map((g) => (
          <line key={g} x1="0" x2={W} y1={H - (g / 100) * H} y2={H - (g / 100) * H} stroke="var(--ds-chart-grid)" strokeWidth="1" strokeDasharray="3 3" />
        ))}
        <polygon points={area} fill={color} opacity="0.12" />
        <polyline points={path} fill="none" stroke={color} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
        <circle cx={last[0]} cy={last[1]} r="3.5" fill={color} />
      </svg>
      {showAxis && <><span className="ds-chart__axis ds-chart__axis--top">100%</span><span className="ds-chart__axis ds-chart__axis--bottom">0%</span></>}
    </div>
  );
}

export function Sparkline({ points = [], color = "var(--ds-accent)", width = 72, height = 22 }) {
