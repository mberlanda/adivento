class FaucetRequestsController < ApplicationController
  ...
  def create
    ...
    if faucet_request.persisted?
      render json: { id: faucet_request.id, status: faucet_request.status, amount_minor: faucet_request.amount_minor },
             status: :created
    else
      render json: { errors: faucet_request.errors.full_messages }, status: :unprocessable_content
    end
  end
